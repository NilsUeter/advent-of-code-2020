...
const unparsedPasswords = unparsedInput
  .split("\n\n")
  .map((e) => e.split(/\s+/).map((e) => e.split(":")));
const input = unparsedPasswords.map((arr) => {
...
const validators = {
  ...
  hgt: (value) => {
    const [number, unit] = [value.slice(0, -2), value.slice(-2)];
    switch (unit) {
      case "cm":
...
const isValid2 = (password) => {
  for (requiredField of requiredFields) {
    const value = password[requiredField];
    if (!value) {
      return false;
    }
