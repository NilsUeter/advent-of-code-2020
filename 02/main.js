let unparsedInput = `1-14 b: bbbbbbbbbbbbbbbbbbb
...
const input = unparsedInput.split("\n").map((x) => {
  const [range, charWithColon, password] = x.split(" ");
  const [min, max] = range.split("-").map((i) => parseInt(i, 10));
...
const part1 = () => {
  ...
  for (const { min, max, char, password } of input) {
...
const part2 = () => {
  let validPasswords = 0;
  for (const { min, max, char, password } of input) {
    const firstPosition = password.charAt(min - 1) === char;
    const secondPosition = password.charAt(max - 1) === char;
    //logical XOR
